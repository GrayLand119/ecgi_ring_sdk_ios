    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Button("Realtime Process") {
                ...
                let filtered = result[0] as! Array<Double>
                let hr = result[1] as? Double ?? 0.0
                
                let desc = String.init(format: "Realtime Result: %d, HR: %.0f", filtered.count, floor(hr))
                print(desc)
            ...
            Button("Diagnose Process") {
                ...
                let filtered2 = diagnose[0] as? Array<Double> ?? [];
                let hrInfo = diagnose[1] as? Array<Double> ?? [];
                let rhythmInfo = diagnose[2] as? Array<Double> ?? [];
                
                let labels = ["正常", "房扑", "房颤", "室颤/室扑", "其他心律不齐", "噪声"]
                // FIXME: ms 单位错误, 实际输出是秒
                print(String.init(format: "minHR:%.2f, meanHR:%.2f, maxHR: %.2f", hrInfo[0], hrInfo[1], hrInfo[2]))
                print(String.init(format: "minRR:%.2fms, meanRR:%.2fms, maxRR: %.2fms", hrInfo[3], hrInfo[4], hrInfo[5]))
                print(String.init(format: "PR间期:%.2fms", hrInfo[6]))
                print(String.init(format: "QRS波宽:%.2fms", hrInfo[7]))
                print(String.init(format: "SDNN:%.2f, RMSSD:%.2f", hrInfo[8], hrInfo[9]))
                print(String.init(format: "诊断结果:%@, 置信度:%.2f", labels[Int(rhythmInfo[0])], rhythmInfo[1]))
                ...
                诊断结果:%@, 置信度:%.2f
"""
                diagnoseResult = String.init(format: fmt,
